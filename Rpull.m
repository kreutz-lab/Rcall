...
if status~=0
    if isfield(OPENR,'myLibPath')
        ...
        error(['Is your R path "' OPENR.Rexe '" correct? You can set the Rpath in Rinit(Rlibraries,Rpath). } Is the R library path correct? You can set it in Rinit(Rlibraries,Rpath,Rlibpaths).'])
    end
end
try
    dat = load('Rpull.mat');
